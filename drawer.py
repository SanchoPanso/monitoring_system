...
class Drawer:
    ...
    def __call__(self,
        ...
        if len(tracker_output.ids) > 0:
            img = self.draw_boxes(img, tracker_output)

        # draw info
        for i, key in enumerate(info.keys()):
            row = f'{key}: {info[key]}'
            t_size = cv2.getTextSize(row, cv2.FONT_HERSHEY_PLAIN, 1.5, 2)[0]
            cv2.putText(img, row, (0, int(t_size[1] * (2 * i + 1.5))),
                        cv2.FONT_HERSHEY_PLAIN, 1.5, (0, 0, 255), thickness=2)
        return img

    def draw_boxes(self, img, tracker_output: TrackerOutput, class_names_list: list = None, offset=(0, 0)):
